...
def projection(lidar: Lidar, camera: Camera, idx: int, filter_outliers=True):
    ...
    camera_pose = camera._poses[idx]
    camera_intrinsics = camera._intrinsics
    ...
    points3d_lidar = lidar._data[idx].to_numpy()[:, :3]
    points3d_camera = trans_lidar_to_camera[:3, :3] @ (points3d_lidar.T) + \
                        trans_lidar_to_camera[:3, 3].reshape(3, 1)

    K = np.eye(3, dtype=np.float64)
    K[0, 0] = camera_intrinsics._fx
    K[1, 1] = camera_intrinsics._fy
    K[0, 2] = camera_intrinsics._cx
    K[1, 2] = camera_intrinsics._cy
    ...
    if filter_outliers:
        image_w, image_h = camera._data[0].size
